...
@section('content')
            ...
            <form action="{{ route('admin.otc_orders.index') }}" method="get" id="filterForm">
                    ...
                    <div class="col-lg-6 col-md-8 col-sm-12 mb-2">
                        ...
                        <x-user-selection-component input-name="user" multiple="0"
                            ...
                            selected-label="{{ request()->filled('user') && $otcOrders->isNotEmpty() && $otcOrders->first()?->user
                                ? '(' .
                                    $otcOrders->first()->user->id .
                                    '#) ' .
                                    $otcOrders->first()->user->fullname() .
                                    ' | ' .
                                    $otcOrders->first()->user->email
                ...
                <div class="row mb-3" id="advancedFilters" style="display: none;">
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6 mb-2">
                        ...
                        <input type="number" name="quantity_min" class="form-control" id="quantity_min" step="any"
                            value="{{ request()->input('quantity_min') }}" placeholder="0">
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-6 mb-2">
                        <label class="form-label" for="quantity_max">مقدار (تا):</label>
                        <input type="number" name="quantity_max" class="form-control" id="quantity_max" step="any"
                            value="{{ request()->input('quantity_max') }}" placeholder="0">
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-6 mb-2">
    ...
    <div class="card mt-3">
                        ...
                        @foreach ($otcOrders as $order)
                            <tr class="table-{{ $order->status->color() }}">
                                ...
                                <td>
                                    ...
                                    @if (
                                        $order->ref_exchange_sell_status &&
                                            $order->ref_exchange_sell_status !== \App\Enums\RefExchangeSellStatusEnum::NOT_REQUIRED)
                                        <br>
                                        <small
                                            class="badge bg-label-{{ $order->ref_exchange_sell_status->color() }} mt-1">
                                ...
                                <td class="sticky-column">
                                    ...
                                    @if (auth()->user()->hasRole('tech_developers'))
                                        <button type="button" class="btn btn-icon btn-text-warning"
                                            ...
                                            title="ثبت نوت">
                                            <i
                                                class="{{ $order->notes ? 'fa-solid' : 'fa-regular' }} fa-note-sticky {{ $order->notes ? 'text-warning' : '' }}"></i>
                                        </button>
                                    @endif
                                </td>
                            </tr>
        ...
        {{-- Transaction Modals --}}
        @foreach ($otcOrders as $order)
            <x-transaction-modal modal-id="otc-{{ $order->id }}" title="تراکنش های معامله #{{ $order->id }}"
                :user="$order->user" :transactions="$order->transactions" route-name="admin.transaction.index" route-param="otc_order_id"
                :route-param-value="$order->id" :ref-exchange-description="$order->ref_exchange_description" />
        @endforeach

        {{-- Developer Notes Modals --}}
        @if (auth()->user()->hasRole('tech_developers'))
            @foreach ($otcOrders as $order)
                <div class="modal fade" id="note-otc-{{ $order->id }}" tabindex="-1" aria-modal="true"
                    role="dialog">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <textarea class="form-control otc-notes-input" rows="5" placeholder="نوت خود را اینجا بنویسید..."
                                    data-id="{{ $order->id }}" data-url="{{ route('admin.otc_orders.notes.update', $order->id) }}">{{ $order->notes }}</textarea>
                            </div>
...
@section('vendor-script')
    ...
    <script>
        $(document).ready(function() {
            ...
            const advancedInputs = ['date_from', 'date_to', 'quantity_min', 'quantity_max', 'total_value_min',
                'total_value_max'
            ];
            ...
            if (hasAdvancedValue) {
                ...
                $('#toggleAdvancedFilter').html(
                    '<i class="fas fa-chevron-up me-1"></i> مخفی کردن فیلترهای پیشرفته');
            }
            ...
            $('.trigger-ref-exchange-sell').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '{{ route('admin.otc_orders.index') }}/' + orderId +
            ...
            $('.reset-ref-exchange-sell').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        button.prop('disabled', true);
                        button.find('i').removeClass('fa-rotate-right').addClass(
                            'fa-spinner fa-spin');
