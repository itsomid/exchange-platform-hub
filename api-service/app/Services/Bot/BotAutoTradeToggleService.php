...
class BotAutoTradeToggleService
{
    ...
    public function setByUser(User $user, bool $enabled): bool
    {
        return $this->setEnabled(
            $user->id,
            $enabled,
            BotAutoTradeEvent::SOURCE_USER,
            BotAutoTradeEvent::REASON_USER_MANUAL,
            $enabled
                ? 'کاربر خرید و فروش خودکار را از تنظیمات بات روشن کرد.'
                : 'کاربر خرید و فروش خودکار را از تنظیمات بات خاموش کرد.',
            BotAutoTradeEvent::ACTOR_USER,
            $user->id,
            $this->userLabel($user->id, $user),
        );
    }

    public function disableByAdminCancelAll(int $userId, ?int $adminId = null, ?string $adminLabel = null): bool
    {
        $reason = 'ادمین همه سفارش‌های باز بات را لغو کرد؛ ربات برای جلوگیری از باز شدن موقعیت جدید خاموش شد.';
        if ($adminLabel) {
            $reason .= ' ادمین: '.$adminLabel.($adminId ? ' (#'.$adminId.')' : '').'.';
        }
        ...
        return $this->setEnabled(
            ...
            BotAutoTradeEvent::REASON_ADMIN_CANCEL_ALL,
            $reason,
            $adminId ? BotAutoTradeEvent::ACTOR_ADMIN : null,
            $adminId,
            $adminLabel,
            ['action' => 'cancel_all'],
        );
    ...
    public function disableBecauseAllBuysFailed(int $userId, int $botOrderId): bool
    {
        return $this->setEnabled(
            $userId,
            false,
            BotAutoTradeEvent::SOURCE_SYSTEM,
            BotAutoTradeEvent::REASON_ORDER_ALL_BUYS_FAILED,
            "تمام سیگنال‌های خرید سفارش بات #{$botOrderId} ناموفق بود و سفارش FAILED شد؛ ربات برای جلوگیری از تکرار چرخه ناموفق خاموش شد.",
            null,
            null,
            null,
            ['bot_order_id' => $botOrderId],
        );
    }
